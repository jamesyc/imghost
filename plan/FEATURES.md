...
- Authenticated users can append additional files to an existing owned album they already control from the owner album workspace in the browser UI, and API clients can still do so by supplying the target album ID during upload.
...
- Anonymous upload responses now include a dedicated `manage_url` so the browser or user can keep a real token-backed management link separate from the public share page.
...
- Public album pages now use the shared template shell, expose ZIP and split-link media actions, show full image previews, and provide an owner-only link back to the private album workspace.
- Public album pages can also surface a `Manage Album` entry point for anonymous uploads when that browser has the saved token for the album.
- Public user album pages now use the same shared page system rather than the earlier inline utility HTML.
